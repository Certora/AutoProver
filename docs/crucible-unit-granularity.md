...
> **Status: SUPERSEDED by [crucible-component-units.md](./crucible-component-units.md).** Solana
> units are now one per **`ProgramComponent`** — a semantic capability produced by system analysis,
> mirroring EVM. Both designs this note weighed are gone: per-instruction (an instruction is a
> syntactic artifact, not a unit of behavior) and the whole-program singleton it settled on (one
> extraction agent is a hard cap on depth — measured on a 62-instruction program in that note's
> §15). The `Ecosystem.collapse_units` / `global_extraction` hooks named below never existed under
> those names and are gone regardless. **The analysis in §4–§7 is still the reference** for *why*
> a fuzzer's unit is an authoring scope rather than an execution scope, and §7's `--binary-in`
> parallelism argument is unchanged and now more load-bearing. The rest is history.
>
> Superseded status header follows.
>
...
- **EVM symmetry:** should Solana analysis gain a `ContractComponent`-style grouping so
  ...
  typically-smaller instruction sets? — **taken up in
  [crucible-component-units.md](./crucible-component-units.md)**, which proposes exactly that
  grouping (review feedback: "instruction" is the wrong unit; the unit should be an abstract
  component) and works through the model, prompt, validation, wheel and cost changes.
