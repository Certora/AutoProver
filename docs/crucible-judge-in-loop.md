...
- **A review budget** (`MAX_REVIEW_ROUNDS`, shared by the hook and the gate). The loop above assumes
  the author *can* address the feedback; when it can't, "revise and re-review" doesn't converge — it
  re-spends a whole, and steadily growing, context per round until the graph's recursion limit. That
  is not hypothetical: on a large program the reviewer's objection was that the shared fixture lacked
  the actions ~20 properties needed, which the component author is not allowed to change, and the
  session spun 11+ rounds at ~340k input tokens each. After the budget the draft goes forward with
  the open concerns recorded in its feedback (and a host-side warning); the compile gate and the
  fuzzer still judge it. Only the reviewer loses the ability to block.
