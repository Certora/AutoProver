...
|---|---|---|
| Phase enum `P` | host, synthesized from `descriptor.phases` | `BuildPreflight ∥ Analysis → Extraction → HarnessFixture(setup) → Formalization → Report` (Preflight and Fixture are UI-only phases, cf. autoprove's harness phase; the preflight overlaps analysis — §5.0) |
...
### 5.0 `preflight` — build the program + IDL, then gate a skeleton harness
...
*authored* fixture — after the whole extraction phase, and as compiler errors the author cannot act
on because it does not own the manifest. The gate instead proves, up front and for ~one analysis
agent's worth of elapsed time: the pinned crucible/libafl/anchor/solana graph resolves *and compiles*;
...
field names, PDA seeds, signer sets, whether an init sequence succeeds. The skeleton sends no
instructions; that is §5.2's job.

This is a **UI-only "Build Preflight" phase** with its own `TaskInfo`, created *unmetered* (a build is
not an agent, so it must not spend one of the run's `--max-concurrent` slots for the whole of
analysis). Its outputs — the resolved program crate and the placed IDL path — flow forward to
`prepare_system` as immutable state (`RustPreflight`).

`prepare_system` itself is then Foundry's identity transform: `locate_main` (from the `SOLANA` chain)
picks the target `SolanaProgramInstance`.
...
model + source **and the extracted properties**, author the **shared harness scaffold** that every
per-component test reuses:

> **It is authored with the properties in hand.** `prepare_formalization` runs *concurrently with
> extraction*, so the properties do not exist yet at that point — it therefore returns a
> `StagedFormalizer`, whose `begin` authors the fixture between extraction and the per-unit fan-out,
> when they do, and returns the formalizer built around it. This matters because
> the fixture decides what is checkable *at all*: tests may read state but never send instructions (an
> invariant that mutates state corrupts the campaign), so an instruction with no `action_*` is a
> property that can only be "covered" by an assertion that holds whatever the program does. Authored
> blind against a large real-world program, it produced 13 actions for 63 instructions, one actor, and
> `u64::MAX` limits — and the reviewer then (correctly) rejected 29 of 37 properties as vacuous or
> unreachable, in a loop the component author had no power to escape.
>
> The build it would otherwise have been serialized behind is not waiting on this at all: the whole
> workspace prep *and its gate* moved earlier still, into §5.0's preflight, where it overlaps system
> analysis instead. So by the time the fixture is authored, the crate it must compile in is already
> known to compile.
...
For each extracted component (a `SolanaComponentInstance` — see §10 Q1), author
...
- **A buildable target program**: a Cargo/Anchor workspace (mirror Foundry's `foundry.toml`-exists
  precondition) whose main-contract path lies inside a crate — the harness path-depends on that
  crate, so the host resolves its directory and Cargo names from the nearest `Cargo.toml`
  (`composer/spec/cargo.py`) rather than assuming `programs/<name>/`: a workspace member's
  directory, package name and the analysis identifier need not agree (lend's is `programs/lend`,
  package `example_lending`), and guessing breaks the build with a "failed to load manifest for
  dependency" error.
...
- **The target program's Solana version may differ from the fuzzer's.** Crucible's own docs call this
  ...
  **from the IDL, with no crate dependency on the program**, so the harness need not compile against the
  program's Solana version. This is **implemented** — see §6.2. (Orthogonal axis: `litesvm` must still be
  able to *load and execute* the program's compiled `.so` — i.e. loader/sBPF compatibility between the
...
- **Two Rust toolchains, not one.** The **harness** is a *native* build — Crucible forces
  ...
  — so it needs a host `stable` recent enough for that Crucible version's MSRV. The generated crate
  therefore ships its own `rust-toolchain.toml` pinning that channel: rustup selects by *directory*
  and the harness lives inside the target project, so otherwise the project's pin would silently
  claim every *other* cargo invocation in there — dependency warming, or a hand-run `cargo build` —
  and an older cargo may not even parse a dependency's manifest (one real program pinned 1.74, which rejects an
  edition-2024 dep outright). The **program `.so`** is
...
| | **crate path** | **IDL path** |
| --- | --- | --- |
| types from | a path dep on the program's crate | `declare_fuzz_program!` over its IDL |
| extra deps | — | `crucible-idl-gen`, `bytemuck`, `ctor` |
| requires | the program's Anchor major == ours | an IDL (any Anchor format) |
...
- **The trait bounds can't be satisfied.** `crucible-test-context`'s builder is bounded by
  `anchor_lang::{InstructionData, ToAccountMetas}` from *its* `anchor-lang` (1.0.1). A program's
  generated impls belong to the exact `anchor-lang` that generated them, so a 0.29 program's
  `instruction::Foo` is a different trait's implementor — no pin makes it fit.
- **The graphs can't even co-resolve.** Solana 1.17's `solana-frozen-abi` pins `ahash "=0.8.5"`, while
  `libafl 0.15` (which `crucible-fuzz-runtime` requires) needs `^0.8.11`. Same minor, disjoint
  requirements. A manifest containing *only* the program crate and libafl already fails to resolve.

Neither is affected by the versions this backend declares, which is why the escape hatch is structural:
under the IDL path the program's dependency graph is never resolved at all, and the generated types
belong to *our* `anchor-lang`. The program's `.so` is still built and loaded by LiteSVM either way —
that's an sBPF artifact, not a linked crate.

Getting the IDL needs the *program's own* `anchor` CLI (`anchor idl build`), which for an older program
is usually not the installed one. `--program-idl <file>` supplies one instead — any Anchor IDL format,
including the pre-0.30 layout (`crucible-idl-gen` runs the same conversion `anchor idl convert` does).
It also forces the IDL path for a program that would otherwise be linked directly. Without either, the
run fails fast in prep with that instruction rather than deep in a build.

One normalization happens on the way in ([solana/build.py](../composer/spec/solana/build.py)): an IDL
must name the program's address, because that is the id the harness sends instructions to — and
`anchor idl build` under 0.29 writes **neither** `address` nor `metadata.address` (only the deploy flow
filled it in), so its output is rejected by the generator as-is. The host fills it from
`Anchor.toml`'s `[programs.<cluster>]` (keyed by the crate's directory, package or lib name, preferring
a local-validator entry), else from a lone `declare_id!` in the crate's source. Several `declare_id!`s
and no manifest entry is an error, not a coin flip — one real program has a feature-gated `staging` id beside the
real one, and picking wrong would fuzz an address the VM never loaded.

### 6.3 What an older program needs installed

The two builds have separate toolchain requirements (§6.1's third axis), and for a pre-0.30 program
neither is the current default. Worked example, a real lending program (`anchor-lang 0.29`, `solana ~1.17.18`):

- **The `.so`** needs an era-appropriate `cargo-build-sbf`, because its platform-tools carry their own
  rustc: that stack pins `ahash 0.8.5`, which uses the `stdsimd` feature removed in Rust 1.78, so
  platform-tools ≥ v1.52 (rustc 1.89) cannot compile it — `error[E0635]: unknown feature 'stdsimd'`.
  Solana CLI **1.18.26** (platform-tools v1.41, rustc 1.75) builds it. Install it out-of-tree and put
  it first on `PATH` for the run rather than switching the active release:
  `~/.local/opt/solana-1.18.26/bin`. (Note `cargo-build-sbf` re-links the `solana` *rustup* toolchain
  to whichever platform-tools it runs — a global side effect either direction.)
- **The IDL** needs the program's own anchor: `avm install 0.29.0`, then
  `anchor-0.29.0 idl build -o target/idl/<lib>.json` from the program's directory (create `target/idl/`
  first — 0.29 doesn't). Call the versioned binary directly; avm's `anchor` shim resolves the version
  from the project but then hangs. Pass the result with `--program-idl` so the run doesn't try to build
  an IDL itself.

The harness build needs neither: it is a native build against Crucible's own pinned stack.
...
(§5.0): `cargo build-sbf` on *user-supplied* program source (and, for a Prover-style backend, on
...
1. **Build + dry-run gate (cheap, no LLM):** provision `crucible` + the sBPF toolchain; assert the
   `preflight` (§5.0) builds the `.so`, generates the IDL, and passes `crucible run --dry-run` on the
   wheel-authored skeleton. This alone validates §6 + §7.1 — and needs no LLM precisely because the
   skeleton is the wheel's, which is why the preflight doubles as the cheap gate.
...
1. **Unit granularity for a fuzzer.** — **RESOLVED: one unit per `ProgramComponent`** (see
   [crucible-component-units.md](./crucible-component-units.md); it supersedes
   [crucible-unit-granularity.md](./crucible-unit-granularity.md), whose per-invariant/whole-program
   answers — and the `global_extraction` / `extraction_unit` / `property_unit` hook names — are
   gone). A component is a semantic capability produced by system analysis, the Solana analog of
   EVM's `ContractComponent`; the split lives on the *ecosystem* axis, so a future CVLR backend
   inherits it too, and each backend aggregates within a unit however suits it. Crucible authors one
   `#[invariant_test] fn c_<component>` per unit and fuzzes the whole program against it — the
   component is an authoring and attribution scope, not an execution scope, exactly as with
   Foundry's stateful `invariant_*`. Original framing retained below for history: the `units` were
   per-instruction, but a Crucible invariant is inherently *cross-instruction* (checked over a
