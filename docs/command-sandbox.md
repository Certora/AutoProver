...
flip:** `run-confined` must be resolvable — `$RUN_CONFINED_BIN`, then PATH (a development
checkout gets it in `.venv/bin` from `uv sync`, which builds `rust/run-confined` as a bin
wheel), then a bare `cargo build -p run-confined --release`. Containers opt in via the `scripts/docker-compose.sandbox.yml`
