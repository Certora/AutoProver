...
2. `main_contract` — `path:ProgramName`. The **path** is what locates the program: its crate
   (directory, Cargo package, lib name) is read from the nearest `Cargo.toml`, so it need not be
   `programs/<Name>/` and the names need not match. `Name` is the analysis identifier — here
   `vault` (**not** the `#[program]` module `vault_program`).
...
- `--cache-ns NAME` — **enables cross-run caching** (off by default). Every expensive step — the
  system analysis, each property-extraction round, the shared fixture, each formalized component —
  is stored under this namespace and reused by a later run with the same `NAME`, so a run that dies
  partway (a rate limit, a toolchain problem) resumes instead of starting over. Reuse is keyed by
  what each step is derived from: the design doc's bytes + the target path root the whole tree, so
  editing the doc or retargeting invalidates it. Editing a *prompt* does not — clear the namespace
  for that.
...
1. **Build Preflight** — `cargo fetch` warms the harness crate, `cargo-build-sbf` compiles the
   program (and `anchor idl build` its IDL, if needed), then `crucible run … --dry-run` builds a
   *skeleton* harness the backend writes itself. This runs **alongside** steps 2–3, and if it fails
   the run stops there rather than authoring against a workspace that doesn't build.
2. **System Analysis** — the model maps the program's instructions/accounts.
3. **Property extraction** — per component of the main program.
4. **Harness Fixture** — the shared-fixture setup session authors the real fixture (with every
   extracted property in hand) and validates it with `crucible run … --dry-run` (labeled e.g.
   `crucible authoring turn`).
