...
- **Per-component units.** `units` returns one `SolanaComponentInstance` per `ProgramComponent`
  ...
  backend-neutral grounds rather than to suit a fuzzer's cost model. The full design note — the
  per-backend analysis and the measurements behind this choice — is
  [crucible-component-units.md](./crucible-component-units.md).
