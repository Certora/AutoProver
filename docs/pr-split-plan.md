...
- **Code:** `composer/crucible/*`, `rust/crucible-app`, `test_scenarios/solana_vault`,
  ...
  **sandbox default → `launcher` for crucible** (fail-closed; the launcher itself is
  ...
  in-container vertical). The `composer/sandbox/recipes.py` grants moved down to PR 2 —
  they are what *any* sandboxed Rust build needs, not Crucible's alone.
...
delta, and it belongs wholly to PR 2, so no sandbox file needs intermediate/final staging.

**Re-split after the branches diverged.** Crucible work kept improving the layer beneath it,
