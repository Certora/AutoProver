...
use autoprover_sdk::authoring::{AuthorInput, Property};
use autoprover_sdk::chain::ChainData;

/// The namespace every name a **run** derives lives in. Prefixed onto a host slug by [`module_of`]
/// (a component's module and section file) and [`check_target`] (a check's Cargo feature and
/// crate-root entry fn) — so nothing else in the crate may start with it.
///
...
/// Fallback module name, used when the host sends a unit with no `slug` — i.e. a single-unit
/// host, where a collision is impossible by construction. A multi-unit host (the Solana ecosystem's
/// per-component units) always supplies one; see [`module_for`]. In the component namespace,
/// because it names a component's module.
pub(crate) const DEFAULT_HARNESS_FN: &str = "c_invariants";

/// The prefix of every authored check fn, and of the build target that selects it.
///
/// The author writes one `pub fn <CHECK_PREFIX><property slug>` per property rather than one fn per
/// component, because a build target is a check (docs/per-check-targets.md). That makes the authored
/// name *vary*, which the old single constant existed to avoid — so what keeps a misnamed fn from
/// being a silent no-op is the build: the crate root generates an entry per property that calls the
/// fn by name, and a name the author did not write is `E0425` naming it, at the gate, before a
/// campaign is spent.
pub(crate) const CHECK_PREFIX: &str = COMPONENT_PREFIX;
...
/// The part of the host's unit object this wheel reads. The unit carries much more (instructions,
/// rationale); the slug decides what its module is called, and the name is what its properties
/// name themselves by ([`Property::component`]).
...
/// One component's place in the crate: the module holding its authored checks, and the build target
/// each of those checks gets.
///
/// Both halves are derived from host slugs, so the crate root can declare every target before a
/// single check is written — which is what lets a target be a *check* rather than a component even
/// though checks are declared later. A check is named for the property it carries
/// ([`check_target`]), and the run's properties are known when the root is rendered.
pub(crate) struct UnitTargets {
    pub(crate) module: String,
    pub(crate) checks: Vec<String>,
}

/// [`UnitTargets`] for one unit, from the run's whole property set.
///
/// A property names its unit by display name, so that is what selects them. A unit with no name is
/// a single-unit host's, where every property is its own and there is nothing to select against.
pub(crate) fn unit_targets(unit: &ChainData, props: &[Property]) -> UnitTargets {
    let unit: UnitRef = unit.parse().unwrap_or_default();
    UnitTargets {
        module: module_of(&unit.slug),
        checks: props
            .iter()
            .filter(|p| unit.name.is_empty() || p.component == unit.name)
...
/// construction.
pub(crate) fn module_of(slug: &str) -> String {
    if slug.is_empty() {
...
/// The Cargo feature — and the crate-root entry fn, and the `crucible run <program> <target>`
/// selector — that one **check** builds under, from the host's slug for the property it carries.
///
/// Same speller as [`module_of`] and the same `c_` namespace, because these are the two things a
/// run names and both must be legal Rust identifiers. A module and an entry fn may share a name
/// (Rust puts them in different namespaces), so a property titled after its own component is not a
/// collision.
pub(crate) fn check_target(prop_slug: &str) -> String {
    format!("{CHECK_PREFIX}{}", ident_of(prop_slug))
}

/// [`module_of`] for a unit as the run-level callouts receive it — `finalize` naming the one behind
/// a component that gave up. Same rule as [`module_for`], reached from the other direction, so a
/// section file and the module declaring it cannot disagree.
pub(crate) fn module_of_unit(unit: &ChainData) -> String {
    module_of(&unit.parse::<UnitRef>().unwrap_or_default().slug)
}

/// [`module_of`] for the unit on an authoring/gating callout.
pub(crate) fn module_for(input: &AuthorInput) -> String {
    let unit = input
        .unit()
...
mod tests {
    ...
    use crate::testkit::{at, component_input, scaffold};
    use autoprover_sdk::authoring::Authored;
    use autoprover_solana::SolanaSourceUnit;

    #[test]
    fn the_module_name_comes_from_the_unit_slug_not_a_re_slugified_name() {
        ...
        assert_eq!(module_for(&input), "c_withdraw_queue");
        assert_eq!(unit_name(&input), "Withdraw Queue");
    }

    #[test]
    fn the_module_name_is_a_valid_snake_case_rust_ident() {
        ...
        assert_eq!(module_for(&capitals), "c_vault_initialization");

        let hyphen = component_input("Admin-Config", "Admin-Config", vec![]);
        assert_eq!(module_for(&hyphen), "c_admin_config");

        // A check's name IS a Cargo feature, a crate-root entry fn and the `crucible run` selector,
        // so it goes through the same speller — from the property's slug, which the host guarantees
        // no more than filesystem-safe either.
        assert_eq!(check_target("Fifo-Order Holds"), "c_fifo_order_holds");

        for ident in ["c_vault_initialization", "c_admin_config", "c_fifo_order_holds"] {
    ...
    fn a_unit_without_a_slug_falls_back_to_the_single_harness_name() {
        ...
        assert_eq!(module_for(&input), DEFAULT_HARNESS_FN);
        assert_eq!(unit_name(&input), "lending");
    }

    #[test]
    fn the_scaffolding_and_component_namespaces_cannot_collide() {
        // Every derived name goes through COMPONENT_PREFIX — so keeping
        ...
        for slug in ["preflight", "main", "invariants", "Preflight", "pre-flight", "a"] {
            assert!(
                module_of(slug).starts_with(COMPONENT_PREFIX),
                "component module {slug:?} escaped the component namespace",
            );
        }
        assert!(module_of("").starts_with(COMPONENT_PREFIX), "the fallback is in the namespace");
        for slug in ["preflight", "main", "a"] {
            assert!(
                check_target(slug).starts_with(COMPONENT_PREFIX),
                "check target {slug:?} escaped the namespace",
            );
        }
        ...
        for slug in ["preflight", "main", "a"] {
            assert_ne!(
                ...
                spec.section_path(&module_of(slug)),
