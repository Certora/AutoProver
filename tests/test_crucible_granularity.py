...
def test_solana_ecosystem_uses_whole_program_extraction():
    from composer.pipeline.ecosystem import SOLANA

    main = SolanaProgramInstance(0, _app())
    # Whole-program mode: SOLANA.units returns a singleton [main] — the whole program is its one
    # unit, so all invariants are inferred + formalized in a single harness + run.
    assert SOLANA.units(main) == [main]
...
@pytest.mark.asyncio
async def test_whole_program_extraction_makes_one_batch(monkeypatch):
    # docs/crucible-unit-granularity.md §3: whole-program mode — units() returns a singleton, so
    # ALL invariants land in ONE batch on the whole-program unit (single harness + run).
    ...
    # units() returns one whole-program unit → one batch carrying every invariant.
    eco = types.SimpleNamespace(
        property_prompts=types.SimpleNamespace(system="s.j2", initial="i.j2"),
        units=lambda main: [_FakeUnit("program", 0)],
    )
