...
from composer.pipeline.core import (
    _Batch, ExtractionFailure, ExtractionResult, _settle_extraction, run_pipeline
)
...
def test_a_failed_component_does_not_cost_the_others():
    ...
    settled = _settle_extraction(
        [good, bad, also_good],
        [_batch(good), boom, _batch(also_good)],  # type: ignore[list-item]
    )

    assert [b.feat for b in settled.batches] == [good, also_good]
    assert settled.failed == [ExtractionFailure(bad, boom)]
...
def test_a_component_with_no_properties_is_not_a_failure():
    ...
    quiet = _Unit("quiet")
    assert _settle_extraction([quiet], [None]) == ExtractionResult([], [])
...
def test_every_component_failing_is_reported_not_hidden():
    ...
    assert settled.batches == []
    assert [f.unit for f in settled.failed] == [one, two]
...
async def test_a_total_extraction_wipeout_names_the_failures(monkeypatch):
    ...
    async def fake_extract_all(*_a, **_kw):
        return ExtractionResult([], [ExtractionFailure(lost, RuntimeError("Internal server error"))])
