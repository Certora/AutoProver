...
async def test_crucible_full_vertical(pg_container: "PostgresContainer", monkeypatch, tmp_path):
    ...
    args = _model_args()
    # Built BEFORE the connections: `standard_connections` takes the resolved provider service (it
    # asks it for an uploader and the memory tool), not a provider name.
    _tiered = get_provider_for(tiered=cast(Any, args))
    async with (
        standard_connections(
            provider=_tiered.provider_service,
    ...
    # ... and the crate the user is left with actually builds.
    #
    # Nothing else covers this. The gated builds (`compile` / `validate`) assemble fixture + spec
    # themselves and never render the deliverable, so a defect in `finalize`'s fold reaches the user
    ...
    # own `#[cfg(feature = "<fn>")] fn main()` and its own `#[global_allocator]`, so enabling two at
    # once is a duplicate-`main` / duplicate-allocator error by construction. A feature selects a
    # fuzz target — `crucible run <program> <fn>` builds with exactly one — so this loop is how the
    # crate is actually used, and it is what proves every delivered component's fn compiles rather
    # than just the first. Reuse the run's private homes so each is an incremental rebuild
    # (identical toolchain + dependency paths ⇒ identical fingerprints), not a cold build.
    env = {
        **os.environ,
        "CARGO_HOME": str(sandbox_cargo_home(scenario)),
        "RUSTUP_HOME": str(sandbox_rustup_home(scenario)),
        "CARGO_NET_OFFLINE": "true",
    }
    for feature in features:
        build = subprocess.run(
            ["cargo", "build", "--release", f"--features={feature}"],
            cwd=crate, capture_output=True, text=True, timeout=1800, env=env,
        )
        assert build.returncode == 0, (
            f"the delivered crate does not compile for feature {feature!r} "
            f"(of {features}):\n{build.stderr[-4000:]}"
        )
