"""A read-only view of the CVLR crate trees a build resolved.

Split out from :mod:`composer.spec.cvlr.source_tools` because the two concerns do not co-vary and
one of them is much cheaper than the other: *presenting several crate trees under one namespace* is
pathlib, while *offering them to an agent* needs the whole tool stack. Keeping the presentation here
means a consumer that only wants to read crate sources — the corpus producer in the private
``certora-cvlr-kb`` repo, which runs as a plain script — can import it without langchain.

The two decisions that make this more than a path walk:
...
``cvlr-asserts``, ``clog!`` in ``cvlr-log``, ``nondet()`` in ``cvlr-nondet``. A consumer handed only
the crate the manifest names would find a re-export and stop.

Only ``*.rs``, ``Cargo.toml`` and ``*.md`` are listed — a registry checkout also carries a lockfile
and cargo bookkeeping that answer no question anyone will ask.
"""
