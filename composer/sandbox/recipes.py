...
def rust_build_policy(
    ...
    *,
    ...
    if offline:
        # Spelled `true`, not `1`: cargo parses this as a strict bool and older versions reject
        # anything else outright ("provided string was not `true` or `false`"). The target
        # project picks the cargo — one real program's `rust-toolchain.toml` pins 1.74 — so this must be
        # the spelling every version accepts, not just the one the harness build uses.
