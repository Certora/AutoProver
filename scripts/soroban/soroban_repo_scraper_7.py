...
========================
Generate a Markdown or JSON contract-function summary from a Soroban/Stellar
GitHub repository, without requiring any local clone or GitHub API
authentication.  Uses only raw.githubusercontent.com for file fetching.
 
Handles two layouts automatically, or allows Cargo.toml directories to be listed
  ...
  • Custom repos – provide a list of root-relative directories containing Cargo.toml files with --cargo_dirs

Note that if you list Cargo.toml files that are contained within a workspace, they will be skipped.
...
Examples:
    ...
    python soroban_repo_reporter.py https://github.com/owner/repo --cargo_dirs thing1/this thing2/that
...
_WORKSPACE_SUBDIRS = ("contracts", "packages", "crates", "src", "stellar", "stellar-contracts")
...
def _default_branch(owner: str, repo: str) -> str:
    for b in ("dev", "develop", "main", "master"):
        for f in ("Cargo.toml", "README.md", "LICENSE", "Makefile"):
            r = _session.head(
                f"{RAW_BASE}/{owner}/{repo}/{b}/{f}", timeout=10
            )
...
def _find_cargo_tomls(
        owner: str, repo: str, branch: str, cargo_check_subdirs: list[str]
) -> list[tuple[str, str]]:
    """
    Locate the workspace/package Cargo.toml.
    Tries the repo root first, then common subdirectories.
 
    Returns (cargo_toml_text, workspace_root) where workspace_root is the
    repo-relative prefix that must be prepended to member paths
    (e.g. "" for root, "contracts" for contracts/Cargo.toml).
    """
    res :list[tuple[str, str]] = [(text, subdir) for subdir in cargo_check_subdirs if (text := _fetch_raw(owner, repo, branch, f"{subdir}/Cargo.toml"))]
...
def _local_package_names_from_lock(
    ...
    `lock_root` is the repo-relative directory that contains Cargo.lock
    (empty string = repo root).
    Returns a list of package names.
    """
    lock_path = f"{lock_root}/Cargo.lock" if lock_root else "Cargo.lock"
    lock_text = _fetch_raw(owner, repo, branch, lock_path)
    if not lock_text:
        return []
    names: list[str] = []
...
def _expand_glob_member(
    ...
    Strategy (tried in order):
      ...
      2. Fall back to Cargo.lock: probe {parent}/{name}/Cargo.toml for every
         local package name found in Cargo.lock (searching from lock_root).
    """
    ...
    local_names = _local_package_names_from_lock(owner, repo, branch, lock_root)
    results = []
...
class ImplBlock:
    ...
    contract_fqn: str          # the struct being implemented
    trait_name: Optional[str]   # None for inherent methods
    trait_fqn: Optional[str]   # None for inherent methods
...
def _parse_source(
    ...
    import_map: dict[str, str] = _parse_use_imports(source)
    ...
    while i < n:
        ...
        # ── #[contracttrait] / #[contractclient] pub trait Name { … } ─────────
        # Both annotations mark a trait as the public contract interface.
        # #[contracttrait]           – OZ-style explicit interface marker
        # #[contractclient(name=…)]  – Soroban SDK annotation that also
        #                              generates a typed client struct
        _IFACE_PREFIXES = ("contracttrait", "contractclient")
        if any(a.startswith(_IFACE_PREFIXES) for a in attrs):
            ...
            if tm:
                ...
                annotation  = "#[" + next(
                    a for a in attrs if a.startswith(_IFACE_PREFIXES)
                ) + "]"
                body_start  = i
                ...
                traits.append(ContractTrait(
                    ...
                    fqn=import_map[trait_name] if trait_name in import_map else trait_name,
        ...
        if not attrs:
            ...
            if tm:
                ...
                traits.append(ContractTrait(
                    ...
                    fqn=import_map[tm.group(1)] if tm.group(1) in import_map else tm.group(1),
                    functions=fns, doc=doc, source_file=filename,
                    span=Span(Pos(start_line, start_col), Pos(end_line, end_col)),
                ))
                continue
        ...
        if any(a.startswith("contractimpl") for a in attrs):
            ...
            if im:
                ...
                impls.append(ImplBlock(
                    ...
                    contract_fqn=src_path_to_module(filename) + "::" + contract_name,
                    trait_name=trait_name,
                    trait_fqn=import_map[trait_name] if trait_name in import_map else trait_name,
...
def _generate_json(
    ...
    def trait_to_dict(ct: ContractTrait) -> dict:
        if ct.annotation.startswith("#[contracttrait"):
            kind = "contracttrait"
        elif ct.annotation.startswith("#[contractclient"):
            kind = "contractclient"
        else:
            kind = "pub_trait"
        return {
    ...
    def impl_to_dict(ib: ImplBlock) -> dict:
        return {
            ...
            "trait_fqn": ib.trait_fqn,          # null for inherent methods
            "contract_fqn": ib.contract_fqn,    # null for inherent methods
        ...
        for ib in pkg.impls:
            if ib.contract_name not in contracts:
                contracts[ib.contract_name] = {
                    ...
                    "kind": "contract",
                    "use": src_path_to_module(ib.source_file) + "::" + ib.contract_name if ib.source_file else None,
                    "source_file": ib.source_file or None,
                    "impl_blocks": [],
                }
...
def main() -> None:
    ...
    parser.add_argument("--cargo_dirs", "-c", nargs="+", type=str,
                        default=_WORKSPACE_SUBDIRS,
                        help="list of directoris containing Cargo.toml files") 
    ...
    if args.cargo_dirs:
        subdirs = args.cargo_dirs
    else:
        subdirs = _WORKSPACE_SUBDIRS
    if args.verbose:
    ...
    tomls: list[tuple[str, str]] = _find_cargo_tomls(owner, repo, branch, subdirs)
    if not tomls:
        sys.exit(
            "ERROR: Could not find Cargo.toml at the repo root or in any of: "
            + ", ".join(subdirs)
        )

    for (cargo_text, workspace_root) in tomls:
        ...
        if _is_workspace(cargo_text):
            ...
            # Member paths in the Cargo.toml are relative to workspace_root;
            # prefix them so all paths are repo-relative from here on.
            if workspace_root:
                raw_members = [f"{workspace_root}/{m}" for m in raw_members]
            if args.verbose:
                print(f"Workspace members (repo-relative): {raw_members}", file=sys.stderr)
            ...
            if pkg_name not in package_names:
                package_names.add(pkg_name)
                pkg = _analyse_package(owner, repo, branch, workspace_root, pkg_name, args.verbose)
                packages.append(pkg)

    actual_pkgs = [pkg for pkg in packages if pkg.has_content()]
    if not actual_pkgs:
        sys.exit(
            "ERROR: Could not find any packages in "
            + ", ".join(subdirs)
        )
