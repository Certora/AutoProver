...
pub mod vault_program {
    ...
    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        ...
        let fee = match ctx.accounts.fee_collector {
            Some(_) => amount * FEE_BPS / 10_000,
            None => 0,
        };
        ...
        **ctx.accounts.authority.to_account_info().try_borrow_mut_lamports()? += to_authority;
        if let Some(collector) = &ctx.accounts.fee_collector {
            **collector.to_account_info().try_borrow_mut_lamports()? += fee;
        }
